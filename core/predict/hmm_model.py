...
class HMMModel:
    ...
    def viterbi(self, observations: dict[int, float]):
        ...
        if best_cell not in self.dynamic_cells_prob.keys():
             return max(self.dynamic_cells_prob, key=self.dynamic_cells_prob.get), self.dynamic_cells_prob.get(max(self.dynamic_cells_prob, key=self.dynamic_cells_prob.get))
