...
class HMMModel:
    ...
    def set_dynamic_cells_prob(self, prev_cell: int):
        ...
        weights = {
            "on_path": 10.0,
            "neighbor_of_path": 5.0,
            "neighbor": 1.0
        }

        for cell in self.grid.get(prev_cell, []):
    ...
    def viterbi(self, observations: dict[int, float]):
        ...
        for cell, emission_prob in observations.items():
            ...
            combined_score = (
                self.weight_emission * math.log(max(emission_prob, 1e-12)) +
                self.weight_transmition * math.log(max(transition_prob, 1e-12)) +
                math.log(penalty)
            )
