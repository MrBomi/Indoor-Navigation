...
def get_all_doors_data(building_id: int, floor_id: int) -> dict:
    doors = Door.query.filter_by(building_id=building_id, floor_id=floor_id).all()
    doors_data = []
    for door in doors:
        doors_data.append({
            "id": door.id,
...
def get_doors_coord(building_id: int, floor_id: int) -> dict:
    doors = Door.query.filter_by(building_id=building_id, floor_id=floor_id).all()
    doors_data = []
    for door in doors:
        doors_data.append({
