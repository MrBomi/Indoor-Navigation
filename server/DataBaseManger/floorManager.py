...
def add_floor(building_id: int, floor_id: str, svg_data: str, grid_svg: str, graph_dict: dict, doors_dict: dict, x_min: float, x_max: float, y_min: float, y_max: float) -> bool:
    try:
        ...
        graph_ok = save_graph_to_db(floor_id, graph_dict)
        doors_ok = save_doors_to_db(doors_dict, floor_id)
...
def get_Svg_data(building_id: int, floor_id: str) -> str:
    try:
...
def get_grid_svg(building_id: int, floor_id: str) -> str:
    try:
        floor = Floor.query.get((floor_id, building_id))
        if floor:
...
def get_floor_by_pk(building_id: int, floor_id: str):
    floor = Floor.query.get((floor_id, building_id))
    if not floor:
        raise ValueError(f"Floor with ID {floor_id} in building {building_id} not found.")
...
def get_all_floor_ids(building_id: int):
    ...
    return [str(floor.id) for floor in floors]

def update_svg_data(building_id: int, floor_id: str, svg_data: str) -> bool:
    try:
        floor = Floor.query.get((floor_id, building_id))
        if not floor:
            raise ValueError(f"Floor with ID {floor_id} in building {building_id} not found.")
...
def update_grid_svg_data(building_id: int, floor_id: str, grid_svg: str) -> bool:
    try:
        floor = Floor.query.get((floor_id, building_id))
        if not floor:
            raise ValueError(f"Floor with ID {floor_id} in building {building_id} not found.")
