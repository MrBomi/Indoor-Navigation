...
FINGERPRINT_CSV = "Postioning/WastonFloor1.csv"   # training fingerprints
TEST_CSV        = "Postioning/WastonTest_aligned_to_WastonFloor1.csv"     # test scans (one row = one scan)
...
CONNECTION_BETA       = 30       # weight of (1 - similarity) term added to distance (higher => more influence)
...
def top_unique_label_guesses(scan_row, fp_df, ap_cols, top_n=3):
    """
    Return up to 'top_n' UNIQUE labels ordered by increasing connection-aware score.
    """
    scan_vec = scan_row[ap_cols].astype(float).values
    fp_vecs  = fp_df[ap_cols].astype(float).values

    # Compute score to every fingerprint vector
    scores = np.array([
    ...
    order = np.argsort(scores)  # lower score is better
