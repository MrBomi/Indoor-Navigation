...
import androidx.appcompat.app.AppCompatActivity
import com.example.admin.model.Door
import com.example.admin.viewmodel.FloorPlanViewModel
import com.example.wifirssilogger.databinding.ActivityDoorsSelectBinding
...
class FloorPlanMapActivity : AppCompatActivity() {
    ...
    private var doorsJson: MutableMap<Int, String> = mutableMapOf()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Set up overlay with references
        binding.doorOverlay.referencePhotoView = binding.imageMap
        binding.doorOverlay.referenceBtnContinue = binding.continueButton
        binding.doorOverlay.doorsMap = doorsJson
        ...
        binding.continueButton.setOnClickListener {
            viewModel.sendNewDoors(1, doorsJson)
        }

        doors?.let {
            binding.doorOverlay.doors = it
            binding.doorOverlay.invalidate()
        }
    ...
    private fun observeViewModel() {
        ...
        viewModel.uploadResult.observe(this) { result ->
            result.onSuccess {
                Toast.makeText(this, "Door names submitted successfully", Toast.LENGTH_SHORT).show()
                setResult(RESULT_OK)
                finish()
            }.onFailure {
                Toast.makeText(this, "Failed to send door names: ${it.message}", Toast.LENGTH_SHORT).show()
            }
        }
    }
